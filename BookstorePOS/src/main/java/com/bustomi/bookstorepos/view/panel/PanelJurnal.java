...
import com.bustomi.bookstorepos.view.tablemodel.TabelModelJurnal;
 ...
 * @author PacuL
 */
public class PanelJurnal extends javax.swing.JPanel {

    private final TabelModelJurnal modelJurnal;
    private Date mulai;
    private Date sampai;
    private boolean filter=false;
    private List<Jurnal> list;
    
    public PanelJurnal()  {
        
        modelJurnal=new TabelModelJurnal();
        initComponents();
        
        TabelJurnal.setModel(modelJurnal);
        
        TabelJurnal.getColumnModel().getColumn(0).setMaxWidth(50);
        TabelJurnal.getColumnModel().getColumn(0).setCellRenderer(new HurufRender());
        TabelJurnal.getColumnModel().getColumn(1).setCellRenderer(new HurufRender());
        TabelJurnal.getColumnModel().getColumn(2).setCellRenderer(new HurufRender());
        TabelJurnal.getColumnModel().getColumn(3).setCellRenderer(new HurufRender());  
        TabelJurnal.getColumnModel().getColumn(4).setCellRenderer(new HurufRender());         
        TabelJurnal.getColumnModel().getColumn(5).setCellRenderer(new HurufRender()); 
        TabelJurnal.getColumnModel().getColumn(6).setCellRenderer(new HurufRender());           
        TabelJurnal.getColumnModel().getColumn(7).setCellRenderer(new HurufRender()); 
        TabelJurnal.getTableHeader().setAlignmentY(CENTER_ALIGNMENT);
    }
    ...
    private void initComponents() {
        ...
        TabelJurnal.setOpaque(false);
        viewPortX1.add(TabelJurnal);
    ...
    private void buttonMin2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonMin2ActionPerformed
        ...
        if (mulai == null) {
            ...
            filter = true;
            JurnalService service=SpringManager.getInstance().getBean(JurnalService.class);
            List<Jurnal> jurnals=service.findAll(mulai,sampai);
            if (jurnals != null) {
                modelJurnal.load(jurnals);
            } else {
    ...
    private void buttonRed1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonRed1ActionPerformed
        if (!filter) {
        ...
        }else if (modelJurnal.getRowCount() == 0) {
            JOptionPane.showMessageDialog(this, "Daftar Jurnal kosong");
        } else {
            ...
            if (pilih == JOptionPane.YES_OPTION) {
                ...
                try {
                    InputStream inputStream=PanelJurnal.class.getResourceAsStream("/com/bustomi/d3ti/uns/report/jurnal.jasper");

                    JRDataSource dataSource=new JRBeanCollectionDataSource(modelJurnal.getList(), false);
